...
public class ValidationController {
    ...
    public ApiResult<PasswordChangeToken> validateUser(@RequestBody @Valid ValidationUserRequest validationUserRequest) {
        return ApiResult.ok(new PasswordChangeToken(validationService.checkUser(validationUserRequest)));
    }

    @ApiOperation(value = "비밀번호 변경 전 비밀번호 확인 (로그인 후)")
    @PostMapping("password/validation")
    public ApiResult<PasswordChangeToken> validatePasswsord(@RequestBody @Valid ValidationPasswordRequest validationUserRequest) {
        return ApiResult.ok(new PasswordChangeToken(validationService.checkPassword(validationUserRequest)));
    }
}
